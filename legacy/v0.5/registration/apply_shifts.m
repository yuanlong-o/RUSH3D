...
%     t_fe = etime(clock,t_f);
%     disp(['fft: ',num2str(t_fe),'s']);
...
%             P = Np{1};
            parfor ii = 1:lY 
                ...
                if all([length(xx_s),length(yy_s),length(zz_s)] == 1)
                    %M_fin = shift_reconstruct(Yfft{1},shifts_temp(ii).shifts,shifts_temp(ii).diff,options.us_fac,R,C,P,options.boundary,0);
...
%             t_le = etime(clock,t_l);
%             disp(['parf: ',num2str(t_le),'s']);
        otherwise
            parfor ii = 1:lY
                minY = min(Ytc{ii}(:));
                maxY = max(Ytc{ii}(:));
