...
outdir = 'D:\RUSH3Dproject\RUSH3Dresult\0622\rasai148d_1_z287\test3'; % YZ
mkdir(outdir)
...
preprocess_param.center_X = 4002; % the center coordinate x of Light field data (Note that the coordinate is from 0 to N-1 in c++)
...
debgrecon_param.zvrange = [-40,360]; % valid
...
recon_param.downsampling_rate = 3; %% Downsampling rate for PSF
...
recon_param.pixel_size = 4.6e-6/5.761*recon_param.downsampling_rate; 
...
debgrecon_param.view_array = view_array;
debgrecon_param.psf_layer_position = 1 : 4 : debgrecon_param.psf_end;
debgrecon_param.first_index = (debgrecon_param.zvrange(1)-debgrecon_param.zwrange(1))/debgrecon_param.zstep;
debgrecon_param.second_index = (debgrecon_param.zvrange(2)-debgrecon_param.zwrange(1))/debgrecon_param.zstep;

mchannel = debgrecon_param.mchannel;
% load start
tic;
[depsf, debgrecon_param] = psf_load_module(debgrecon_param,debg_psfpath,view_array);
...
time_grbin = min(400,ceil(preprocess_param.valid_frame_num/debgrecon_param.mchannel));

cv_ind = ceil(length(view_array)/2);

global bg_ratio
bg_ratio = gpuArray.zeros(length(view_array),debgrecon_param.mchannel,'single');

[~, ~, seq_ind] = Spiral_circle(preprocess_param.Nnum,preprocess_param.view_range);
...
for channel = 1: debgrecon_param.mchannel
    frame_i = channel;
    view_frame = zeros(size(first_WDF,1),size(first_WDF,2),length(view_array));
    view_video = zeros(size(first_WDF,1),size(first_WDF,2),length(view_array),time_grbin);
    savegroup = 1;
    disp('---------------------------multiscale detrending---------------------------');
    while frame_i <= preprocess_param.valid_frame_num
        
        tic;
        % load viewframe  
        for v = 1: length(view_array)
            view_frame(:,:,v) = imread(sprintf('%s_No%d.tif', realigndata_name_perfix, frame_i - 1),...
                sub2ind([preprocess_param.Nnum,preprocess_param.Nnum],view_array{v}(2),view_array{v}(1)));
        end
        
        % multiscale debg
        view_frame = sub_mul_bg_module(depsf,debgrecon_param,view_frame,...
            vid_recon_mul_savepath,vid_debg_mul_savepath,first_file_name,frame_i-1,channel-1);
        view_video(:,:,:,mod(ceil(frame_i/mchannel)-1,time_grbin)+1) = view_frame;
        t_debg = toc;
        fprintf('%d frame debg done and it takes %.2f secs\n',frame_i,t_debg);
        % reg if timebin is full
        if mod(ceil(frame_i/mchannel),time_grbin) == 0 || frame_i + mchannel > valid_frame_num
            disp('---------------------------Registration---------------------------');
            centerview_video = squeeze(view_video(:,:,cv_ind,:));
            ...
            for v_ind = 1: length(view_array)
                v = seq_ind(v_ind);
                single_view_video = squeeze(view_video(:,:,v,:));
                single_view_video = apply_shifts(single_view_video, shifts, option_r, bound/2, bound/2);
                
                % save
                saveastiff(im2uint16(single_view_video/65535),...
                    sprintf('%s\\reg_view_%d_g_%d.tiff',reg_savepath,v,savegroup));
                
                % calculate std
                maxIter = regstd_param.maxIter;
                if regstd_param.rankdetrending == 1
                    [curr_bg_spatial, curr_bg_temporal] = rank_1_NMF(reshape(single_view_video, [], size(single_view_video,3)), maxIter);
                    curr_std_image = compute_std_image(reshape(single_view_video, [], size(single_view_video,3)), ...
                        curr_bg_spatial(:), curr_bg_temporal);
                else
                    curr_std_image = compute_std_image(reshape(single_view_video, [], size(single_view_video,3)));
                end
                std_WDF(:, :, v) =sqrt( (floor(frame_i/mchannel)*std_WDF(:, :, v).^2 + size(single_view_video,3)*reshape(curr_std_image, [size(view_video , 1), size(view_video , 2)]).^2)...
                    /(floor(frame_i/mchannel)+size(single_view_video,3)) );
            end
            t_reg = toc;
            fprintf('%d view reg done and it takes %.2f secs\n',frame_i,t_reg);
            savegroup = savegroup + 1;
            rest_frame = floor((preprocess_param.valid_frame_num - frame_i)/mchannel);
            view_video = zeros(size(first_WDF,1),size(first_WDF,2),length(view_array),min(time_grbin,rest_frame));
        end
        frame_i  = frame_i + mchannel;
    end
end
t_debg_reg_end = etime(clock,t_debg_reg);
fprintf('debg and reg done and it takes %.2f secs\n',t_debg_reg_end);
debgrecon_param.savegroup = savegroup - 1;
