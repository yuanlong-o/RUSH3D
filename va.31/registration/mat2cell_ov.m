...
for i = 1:length(xx_s)
    for j = 1:length(yy_s)
        for k = 1:length(zz_s)
            extended_grid = [max(xx_s(i)-overlap(1),1),...
                             min(xx_f(i)+overlap(1),sz(1)), ...
                             max(yy_s(j)-overlap(2),1), ...
                             min(yy_f(j)+overlap(2),sz(2)), ...
                             max(zz_s(k)-overlap(3),1), ...
                             min(zz_f(k)+overlap(3),sz(3))];            
            if nd == 2
