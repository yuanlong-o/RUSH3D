function [S_init, S_shell_init,S_mask_init,  S_shell_mask_init,valid_seg] = seed_generation_module(psf, valid_seg, ...
                                                                volume_size, curr_patch_info, specify_wigner, shell_radius, seed_param)
...
for j = 1 : size(valid_seg, 1)
    ...
    % down sampling
    A_in = imresize(A_in,[curr_patch_info.wdf_ds_size(1),curr_patch_info.wdf_ds_size(2)]);
    %A_in = imresize(A_in, 1 / ds_ratio, 'nearest');
    A_in_shell = imresize(A_in_shell,[curr_patch_info.wdf_ds_size(1),curr_patch_info.wdf_ds_size(2)]);
