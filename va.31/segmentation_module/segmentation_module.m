function [valid_seg_global_filt] = segmentation_module(curr_volume, curr_volume_ori, vessel_mask, seed_param)
...
% size
[size_h, size_w, sizez] = size(curr_volume);
[size_hm, size_wm, ~] = size(curr_volume_ori);
psf_layer_position = 1 : sizez;
patch_info_array = determine_patch_size(size_hm, size_wm, estimate_patch_size);
...
for j = 1 : length(patch_info_array) % for lateral patches
    ...
    % grab current patch
    patch_volume = curr_volume_ori(curr_patch_info.location(1, 1) : curr_patch_info.location(2, 1), ...
    ...
    if find(~cellfun(@isempty,valid_seg))
        for k = 1 : size(valid_seg, 1)
            ...
            valid_seg{k, 2}(:, 1) = valid_seg{k, 2}(:, 1) + curr_patch_info.location(1, 1) - 1 - margin;
            valid_seg{k, 2}(:, 2) = valid_seg{k, 2}(:, 2) + curr_patch_info.location(1, 2) - 1 - margin;
        end
...
for i = 1 : size(valid_seg_global, 1)
    ...
    % boundary judge
    if curr_center(1) - size_h > 0 || curr_center(2) - size_w > 0 ||...
...
for i = 1 : size(valid_seg_global_bd_filt, 1)
   ...
   % do the mapping
   curr_center_cut = mean(curr_center(:, 1 : 3), 1); % only xy
%    curr_center_cut = curr_center_cut / ds_ratio;
    ...
    if size(vessel_mask,3)>1
        if vessel_mask(max(min(floor(curr_center_cut(1)), size(vessel_mask,1)), 1), ...
                max(min(round(curr_center_cut(2)), size(vessel_mask,2)), 1), ...
                max(min(floor(curr_center_cut(3)), size(vessel_mask,3)), 1)) > 0 % in the mask
            exclude_ind(i) = 1; % set 0 to cancel other options
        end
    else
        if vessel_mask(max(min(floor(curr_center_cut(1)), size(vessel_mask,1)), 1), ...
                max(min(round(curr_center_cut(2)), size(vessel_mask,2)), 1)) > 0 % in the mask
            exclude_ind(i) = 1; % set 0 to cancel other options
        end
    end
