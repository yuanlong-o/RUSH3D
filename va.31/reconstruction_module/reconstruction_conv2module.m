function [Xguess] = reconstruction_conv2module(recon_param, Xguess, psf, WDF, ...
...
for i=1:maxIter
    ...
    for v_ind = 1: view_num
        ...
        for z = 1: size(tmp2,3)
            projection = projection + conv2(Xguess(:,:,z),tmp2(:,:,z),'same');
        end
        error = blur_image_uv./projection;
        tmp2 = rot90(tmp2,2);
        errorBack =  gpuArray.zeros(size(Xguess,1),size(Xguess,2),size(Xguess,3),'single');
        for z = 1: size(tmp2,3)
            errorBack(:,:,z) = conv2(error,tmp2(:,:,z),'same');
        end
