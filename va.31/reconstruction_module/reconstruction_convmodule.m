function [Xguess] = reconstruction_convmodule(recon_param, Xguess, psf, WDF, ...
...
for i=1:maxIter
    ...
    for v_ind = 1: view_num
        ...
        tmp2 = gpuArray(squeeze(psf(:,:,view,:)));
        tmp3 = flip(tmp2,3);
        projection = convn(Xguess,tmp3,'same');
        projection = projection(:,:,ceil(size(tmp3,3)/2));
        errorBack = blur_image_uv./projection;
        clear tmp3;
        tmp2 = rot90(tmp2,2);
        errorBack = convn(errorBack,tmp2);
        errorBack = errorBack(ceil(size(errorBack,1)/2)-floor(size(Xguess,1)/2):ceil(size(errorBack,1)/2)+floor(size(Xguess,1)/2),...
            ceil(size(errorBack,2)/2)-floor(size(Xguess,2)/2):ceil(size(errorBack,2)/2)+floor(size(Xguess,2)/2),:);
         Xguess = Xguess.*errorBack*weight1(view)*iter_weight+(1-weight1(view)*iter_weight).*Xguess;
         clear errorBack tmp2;
