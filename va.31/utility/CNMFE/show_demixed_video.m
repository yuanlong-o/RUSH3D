function avi_filename = show_demixed_video(obj,Y_mat, save_avi, kt, frame_range, ...
            amp_ac, range_ac, range_Y, multi_factor, output_dir, use_craw)
...
Y = Y_mat(:, :, tmp_range(1) : tmp_range(2));
Ybg = CNMFE_reconstruct_background(obj,Y_mat, tmp_range);
...
%% create avi file
if save_avi
    avi_filename =[output_dir, '/', 'demixed.avi'];
...
for tt=t_begin:kt:t_end
    ...
    img_ac = CNMFE_reshape(obj, obj.A*CC(:, tt), 2);
    ...
    axes(ax_mix); cla;
    imagesc(CNMFE_reshape(obj, Y_mixed(:, m,:),2));  hold on;
    title('Demixed');
