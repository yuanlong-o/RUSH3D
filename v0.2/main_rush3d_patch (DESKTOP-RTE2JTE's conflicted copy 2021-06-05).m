...
[i_index, j_index] = gen_spiral(preprocess_param.Nnum);
reg_group = ceil(preprocess_param.Nnum / estimate_view_num);
...
for reg_ind = 1 : reg_group
    ...
    if reg_ind == 1
        ...
        [~, shifts, bound, option_r] = motion_correction(central_video, d1, d2, ...
            reg_bin_width, outdir);
        max_value = max(central_video(:));
    end
	...
	for view_ind = 1 : length(curr_view_group_i)
        ...
        buf_ind = sub2ind([preprocess_param.Nnum, preprocess_param.Nnum], ...
                            curr_view_group_i(view_ind), curr_view_group_j(view_ind));
        saveastiff(im2uint16(curr_video / max_value / 5), sprintf('%s\\reg_view_%d', reg_path, buf_ind));
        
        % calculate std
        [curr_bg_spatial, curr_bg_temporal] = rank_1_NMF(reshape(curr_video, :, size(curr_video,3)), maxIter);
        curr_std_image = compute_std_image(reshape(curr_video, :, size(curr_video,3)), bg_spatial_init(:), bg_temporal_init);
...
for global_patch_id = 1 : length(patch_info_array) % for lateral patches
    ...
    patch_WDF = first_WDF(curr_patch_info.location(1, 1) : curr_patch_info.location(2, 1), ...
                          curr_patch_info.location(1, 2) : curr_patch_info.location(2, 2), ...
                          :);
    ...
    [S_init, S_shell_init,S_mask_init, S_shell_mask_init] = seed_generation_module(psf(:, :, :, :, start_ind : end_ind), ...
                                                                    psf_param, valid_seg, ...
                                                                    patch_volum_size_cut, specify_wigner, shell_radius);
