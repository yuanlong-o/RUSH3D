function processed_video = iteration_preparation_patch( reg_path, valid_frame, specify_wigner, curr_patch_info, psf_param, S_init)
...
buf_wigner_video = loadtiff(sprintf('%s_view_%d.tif', reg_path, 1));
[size_h_global, size_w_global, ~] = size(buf_wigner_video);

% determine cut position for h
cut_start_h = max(round(curr_patch_info.location(1, 1) / ds_ratio), 1);
...
% the same for w
cut_start_w = max(round(curr_patch_info.location(1, 2) / ds_ratio), 1);
...
for j = 1 : num_wigner % number of specified wigner
    curr_video = loadtiff(sprintf('%s_view_%d.tif', reg_path, specify_wigner(j)));
