...
% commented by YZ. last update: 6/16/2020.
...
memmap = options.memmap; % flag for saving memory
grid_size = options.grid_size;  % grid size
mot_uf = options.mot_uf;  % degree of patches upsampling (default: [4,4,1])
min_patch_size = options.min_patch_size; % minimize patch size
overlap_pre = options.overlap_pre; % size of overlapped region before upsampling
overlap_post = options.overlap_post; % size of overlapped region after upsampling
upd_template = options.upd_template; %  flag for online template updating
bin_width = options.bin_width; % 
buffer_width = options.buffer_width;
max_dev_g = options.max_dev;
init_batch = options.init_batch; % length of init batch
us_fac = options.us_fac; % upsampling factor
method = options.method; % average the template, median or mean
filename = options.mem_filename;
iter = options.iter;
add_value = options.add_value; % flat, add dc value to data
max_shift = options.max_shift;
print_msg = options.print_msg; % flat for printing
...
if nargin < 3 || isempty(template)
    ...
    template_in = median(Y_temp,nd+1)+add_value; % calculate the median to get a template?
    fftTemp = fftn(template_in);
    
    % loop for time stamps
    for t = 1:size(Y_temp,nd+1)        
        ...
        template_in = template_in*(t-1)/t + M_temp/t; % why is that?
    end
    template_in = template_in + add_value; % directly add a dc value?
...
[xx_s,xx_f,yy_s,yy_f,zz_s,zz_f, ...
    xx_us,xx_uf,yy_us,yy_uf,zz_us,zz_uf] = ...
    construct_grid(grid_size,mot_uf,d1,d2,d3,min_patch_size);

% set up shift g
shifts_g = struct('shifts',cell(T,1),'shifts_up',cell(T,1),'diff',cell(T,1));

% output all the patches into cell array, note here use upsampled grid
...
for i = 1:length(xx_us)
    for j = 1:length(yy_us)
        for k = 1:length(zz_us)
            ...
            extended_grid = [max(xx_us(i)-overlap_post(1),1), ...
                min(xx_uf(i)+overlap_post(1),d1), ...
                max(yy_us(j)-overlap_post(2),1), ...
                min(yy_uf(j)+overlap_post(2),d2), ...
                max(zz_us(k)-overlap_post(3),1), ...
                min(zz_uf(k)+overlap_post(3),d3)];          
            
            Bs{i,j,k} = permute(...
                construct_weights([xx_us(i),xx_uf(i),yy_us(j),yy_uf(j),zz_us(k),zz_uf(k)],extended_grid), ...
                [2,1,3]); 
        end
    end
end
...
%                 end
        ...
        parfor ii = 1:lY
            ...
            % cell array record all patches in a single frame.
            ...
            % for fft method, call shift_reconstruct function
            switch lower(options.shifts_method)
                ...
                % otherwise, call imwarp which is built-in in matlab
                otherwise
