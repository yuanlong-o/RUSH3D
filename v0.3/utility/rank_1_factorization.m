function [bg_spatial, bg_temporal] = rank_1_factorization(Y, maxIter, Y_rescale_divisor)
...
d1y = size(Y, 1);
bg_spatial = ones(d1y, 1) / sqrt(d1y);
for iter = 1 : maxIter
    bg_temporal = (bg_spatial' * Y) ./ Y_rescale_divisor;
    bg_temporal = bg_temporal ./ norm(bg_temporal(:));
    bg_spatial = (Y * bg_temporal') ./ Y_rescale_divisor;
    if iter < maxIter
