function [S_init, S_shell_init,S_mask_init,  S_shell_mask_init] = seed_generation_module(psf, psf_param, valid_seg, ...
                                                                volume_size, specify_wigner, shell_radius)
...
for j = 1 : size(valid_seg, 1)
    ...
    for i = 1 : size(specify_wigner, 1)
        curr_u = specify_wigner(i, 1);
        curr_v = specify_wigner(i, 2);

        % sample
        HXguess = prop_to_target_wigner(A_in, psf_d, curr_u, curr_v);
        HXguess_shell = prop_to_target_wigner(A_in_shell, psf_d, curr_u, curr_v);
        
        HXguess_mask = prop_to_target_wigner(single(A_in > 0.1 * max(A_in(:))), psf_d, curr_u, curr_v);
        ...
        % record
        S_init{j, i} = sparse(double(HXguess));
...
%% utility function
function HXguess = prop_to_target_wigner(Xguess, psf_z, curr_u, curr_v, ds_ratio)
    ...
    % pad to large psf
    large_psf = zeros(size(Xguess, 1), size(Xguess, 2), size(tmp, 3));
    large_psf((size_h + 1) / 2 - (size_psf_h - 1) / 2 : (size_h + 1) / 2 + (size_psf_h  - 1) / 2, ... 
              (size_w + 1) / 2 - (size_psf_w - 1) / 2 : (size_w + 1) / 2 + (size_psf_w - 1) / 2, :) = tmp;
