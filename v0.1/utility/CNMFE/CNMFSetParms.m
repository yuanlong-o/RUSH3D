...
% Note here varargin is this structure: first is an option, remains is one name one value
Names = [
    % dataset info
    'd1                 ' % number of rows
    'd2                 ' % number of cols
...
options = [];
for j = 1:m % create options and with all the name
    eval(['options.' Names(j,:) '= [];']);
end
i = 1;
% set the input arg to options, (Or copy the input option structrue to new-built option structure)
while i <= nargin
    arg = varargin{i};
    if ischar(arg), break; end % so here is only for the first parameter, known as input structure
    if ~isempty(arg)                      % [] is a valid options argument
        ...
        for j = 1:m
            if any(strcmp(fieldnames(arg),deblank(Names(j,:)))) % interesting design, see Name , there are so many spaces
                eval(['val = arg.' Names(j,:) ';']); % so here actually require arg has the same field as Names
            else
                val = [];
            end
            if ~isempty(val)
...
expectval = 0;                          % start expecting a name, not a value. because some parameters are value, while others are string or char
while i <= nargin % in matlab, nargin is the keyword for each function. here i may not start from 0
    arg = varargin{i};
    ...
    if ~expectval
        ...
        lowArg = lower(arg); % chage to lower case
        j = strmatch(lowArg,names);
        if isempty(j)                       % if no matches
            error(sprintf('Unrecognized parameter name ''%s''.', arg));
        elseif length(j) > 1                % if more than one match
            % Check for any exact matches (in case any names are subsets of others)
            k = strmatch(lowArg,names,'exact'); % interesting. why do two times match?
            if length(k) == 1
                j = k;
            else
        ...
        expectval = 1;                      % we expect a value next??? but there are parameter need string input
...
Values = [ % while tihs default setup is soooooo stupid
    % dataset info
    {[]}
    {[]}
    {1}
