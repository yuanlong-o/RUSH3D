...
% 增加了SlightRotate
% 更新（20-9-4）
% 增加了modClose，使用modClose将任意中心点转换到1024附近的中心点（需要非中央中心区域时请自行删除函数）
% 更新（20-9-7）
% 修复了realign 32bit(float)类型时的一个bug

RawDir=fullfile('..');
Nnum = 15;%微透镜阵列后的pixel数，一般不需要动
resize = 0;%是否resize到13x13输出
groupMode = 0;%0=间隔模式(1-9,10-18,...);1=连续模式(1-9,2-10,...)
centerX=modClose(1093,1024,15);%中心点X，自动模式下失效，从0到N-1（在c++坐标系，从0开始！） 请填preResize之前的中心点，程序中会自动乘上preResize
centerY=modClose(1062,1024,15);%中心点Y，自动模式下失效，从0到N-1（在c++坐标系，从0开始！）
...
%     imwrite(img, strcat(path,'/UsedImg.tiff'));
    imwriteTFSK(img, strcat(path,'/UsedImg.tiff'));
end
...
function [anss] = modClose(source,target,mods)
    if(nargin < 1)
        fprintf('modCloseSK(source,target,mods)\n');
        return 
    end
    anss = target+(mod(source, mods) - mod(target,mods));
    if(anss < target)
        anss = anss + mods;
    end
end
